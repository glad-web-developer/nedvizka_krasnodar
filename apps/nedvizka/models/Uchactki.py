...
class UchactkiProdaza(models.Model):
    class Meta:
...
class VideoUchactkiProdaza(models.Model):
    class Meta:
        verbose_name = 'Видео'
...
class PanoramiUchactkiProdaza(models.Model):
    class Meta:
...
class UchactkiArenda(models.Model):
    class Meta:
        verbose_name = 'Участки - аренда'
        verbose_name_plural = 'Участки - аренда'
...
class FotoUchactkiArenda(models.Model):
    class Meta:
        verbose_name = 'Фото'
        verbose_name_plural = 'Фото'
        ordering = ['-index_sortirivki']

    nedvizka = models.ForeignKey(UchactkiArenda, verbose_name='Недвижка', on_delete=models.CASCADE, null=True, blank=True,
                            related_name='foto_set')
    img = FilerImageField(verbose_name='Фото', on_delete=models.CASCADE)
    index_sortirivki = models.IntegerField('Индекс сортировки', default=0, help_text='Чем больше тем раньше выведет')
...
class VideoUchactkiArenda(models.Model):
    class Meta:
        verbose_name = 'Видео'
        verbose_name_plural = 'Видео'
        ordering = ['-index_sortirivki']
...
class PanoramiUchactkiArenda(models.Model):
    class Meta:
        verbose_name = 'Панорамы'
        verbose_name_plural = 'Панорамы'
        ordering = ['-index_sortirivki']
